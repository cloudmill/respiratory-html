  ...
  return (
    <ul className="series__slides">
      {slides.map((slide, index) => (
        <li key={index} className="series__slide">
          <Content
            slide={slide}
            state={animation ? (transition ? (transition === 'prev' && index === animation.prev ? ()) : 'none') : index === cur ? "active" : "none"}
          />
        </li>
      ))}
    </ul>
